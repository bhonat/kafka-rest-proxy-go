...
Public benchmark dashboard: https://bhonat.github.io/kafka-rest-proxy-go/

The dashboard is generated by the `Benchmark regression` GitHub Actions
workflow. The workflow publishes only a sanitized HTML report; local paths,
usernames and email addresses are rejected before deployment. For the first
publication, configure GitHub Pages to use GitHub Actions, then run the workflow
from `main`.
